...
<script>
    $('#categories').select2({
        ajax: {
            url: 'http://localhost:8080/larashop/ajax/categories/search',
...
<div class="row">
    <div class="col-md-8">
        ...
        <form enctype="multipart/form-data" method="POST" action="{{route('books.update', ['id' => $book->id])}}"
            ...
            <input type="text" class="form-control {{$errors->first('slug') ? "is-invalid" :""}}" value="{{$book->slug}}" name="slug" placeholder="enter-a-slug" />
            <div class="invalid-feedback">
                {{$errors->first('slug')}}
            </div>
            <br>
            ...
            <select class="form-control" name="categories[]" multiple id="categories"></select>
            <br>
            <br>

            <label for="stock">Stock</label><br>
            <input type="text" class="form-control {{$errors->first('stock') ? "is-invalid" :""}}" placeholder="Stock" id="stock" name="stock"
                value="{{$book->stock}}">
            <div class="invalid-feedback">
                {{$errors->first('stock')}}
            </div>
            ...
            <input class="form-control {{$errors->first('publisher') ? "is-invalid" :""}}" type="text" placeholder="Publisher" name="publisher" id="publisher"
            ...
            <input type="text" class="form-control {{$errors->first('price') ? "is-invalid" :""}}" name="price" placeholder="Price" id="price"
                value="{{$book->price}}">
                <div class="invalid-feedback">
                    {{$errors->first('price')}}
                </div>    
